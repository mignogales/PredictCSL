#!/usr/bin/env bash
# Full TiRex Family-B long-lag perturbation run for the paper plot.
set -euo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$REPO_ROOT"

CONFIG="${INTERPRETABILITY_CONFIG:-context_interpretability/configs/experiments_long_lag_tirex_family_b_full.yaml}"
