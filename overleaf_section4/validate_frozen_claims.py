...
def main() -> None:
    ...
    long_lag = pd.read_csv(DATA / "exp4_long_lag_all_models.csv")
    assert long_lag[long_lag.strength.eq(1.0)].model.nunique() == 9
    ...
    strong = long_lag[long_lag.strength.eq(1.0)]
    assert len(strong) == 36
    assert int((strong.sufficient_context >= strong.lag).sum()) == 28
    assert int(strong.limitation_flag.sum()) == 8
    toto_strong = strong[strong.model.eq("Toto-2.0-313m")].sort_values("lag")
    assert toto_strong.sufficient_context.tolist() == [384, 1024, 1536, 2560]
    assert bool((toto_strong.sufficient_context >= toto_strong.lag).all())
    moderate = long_lag[long_lag.strength.eq(0.5)].sort_values("lag")
    assert moderate.model.unique().tolist() == ["Toto-2.0-313m"]
