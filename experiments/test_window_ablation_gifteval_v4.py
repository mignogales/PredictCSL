"""
Window-size ablation for zero-shot TS foundation models on GiftEval.

Refactor goals
--------------
- Loop order: Model -> Dataset -> Window (load each model into VRAM exactly once).
- Aggressive runtime optimization: precompute per-(dataset,term) tensor pool once,
  slice per window; pinned-memory + non_blocking H2D; metric computation on GPU;
  plots and per-sample I/O skipped on cache hits.
- argparse-based CLI; no Hydra / OmegaConf.
- Granular metrics.json cache per (dataset, model, term, window). Resumable.
- Naive seasonal baseline retained (MASE denominator + reference lines).
- FFT/autocorrelation seasonality detection and seasonality_x2 auto_size logic
  removed entirely.
- Strict context-window validation: if window_size exceeds the longest available
  test-instance context for a dataset, the window is skipped. Test instances
  whose context is shorter than the requested window are dropped (no zero-pad).
"""
...
class GiftEvalCache:
    """
    Iterates `ge_dataset.test_data` exactly once and stores the full contexts
    and labels as numpy arrays. Slicing per window_size becomes O(N*W).

    Also computes the naive seasonal MAE (MASE denominator) and provides
    fast batch construction with strict context-length validation.
    """
    ...
    def __init__(self, ge_dataset: GiftEvalDataset, dataset_display: str):
        ...
        self.labels_np: np.ndarray = np.stack(labels, axis=0)  # (N, H)
    ...
    # ---------- Window-aware batch construction --------------------------------

    def can_serve(self, window_size: int) -> bool:
        """Strict context-window validation: at least one instance has >= ws."""
    ...
    def build_batches(
        ...
        Returns
        -------
        batches : list of dict {"x": (B,W,1) on `device`, "y": (B,H,1) on `device`}
        all_x   : (N_valid, W, 1) on CPU (for plotting only)
        all_y   : (N_valid, H, 1) on CPU
        valid_indices : np.ndarray, indices into the original test pool
        """
        ...
        # Pre-allocate destination once -- avoids per-instance tensor allocation.
        n = valid_indices.size
        x_np = np.empty((n, window_size), dtype=np.float32)
        for j, i in enumerate(valid_indices):
            x_np[j] = self.contexts[i][-window_size:]

        y_np = self.labels_np[valid_indices]  # (n, H)

        all_x = torch.from_numpy(x_np).unsqueeze(-1)  # (n, W, 1) CPU
        all_y = torch.from_numpy(y_np).unsqueeze(-1)  # (n, H, 1) CPU

        # NOTE: batches stay on CPU. Several upstream pipelines (Chronos2 etc.)
        # use their own DataLoader with pin_memory=True internally and fail if
        # they receive CUDA tensors. Each predictor performs its own H2D move.
...
# ==============================================================================
#  NAIVE BASELINE (test-set metrics, cached)
# ==============================================================================
...
def compute_naive_seasonal_test_metrics(
    cache: GiftEvalCache,
) -> Dict[str, float]:
    season = cache.season
...
# ==============================================================================
#  PLOTTING (skipped when --no-plots OR result is cached)
# ==============================================================================
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Window-size ablation on GiftEval.")
    p.add_argument("--batch-size", type=int, default=32, help="Inference batch size.")
    p.add_argument("--window-sizes", type=int, nargs="+", default=None,
                   help="Override default window-size grid.")
    p.add_argument("--models", type=str, nargs="+", default=None,
                   help="Filter by model short name(s).")
    p.add_argument("--datasets", type=str, nargs="+", default=None,
                   help="Filter by dataset display name(s).")
    p.add_argument("--device", type=str, default=None, choices=[None, "cuda", "cpu"],
                   help="Force device. Default: cuda if available.")
    p.add_argument("--no-plots", action="store_true",
                   help="Skip all per-(model,window) plotting (summary plot still produced).")
...
def main():
    ...
    window_sizes = sorted(set(args.window_sizes if args.window_sizes else DEFAULT_WINDOW_SIZES))
    ...
    print(Fore.CYAN + f"Device: {device}  |  windows: {window_sizes}" + Fore.RESET)
    ...
    # Caches shared across all models -- built lazily on first need
    ...
    for model_id, model_family, model_short in models:
        ...
        for ge_name, term, dataset_display, to_univariate in datasets:
            ...
            for window_size in window_sizes:
                ...
                # ----- Per-sample + plots (skipped if --no-plots) -----
                ...
                # Free batch memory before next window
